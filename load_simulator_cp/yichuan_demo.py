...
def calculate_fitness(individual, average_loads):
    ...
    # group_means = [np.mean(loads) for loads in loads_by_group.values() if loads]
    ...
    for satellite_name, group in zip(satellite_names, individual):
        satellite_groups[satellite_name] = f'Group {group}'
...
def dynamic_groups(satellite_load_data, start_time, num_groups):
    ...
    for i in range(num_groups):
        ...
        for satellite in group_satellites:
            ...
            # 将每个卫星的平均负载添加到相应分组
            group_loads[f'Group {i + 1}'].append(average_loads[satellite])

        # 计算各分组平均负载的标准差
    group_average_loads = [np.mean(group_loads[group]) for group in group_loads]
    std_dev = np.std(group_average_loads)

    print(f'dynamic_groups: {std_dev}')
...
if __name__=='__main__':
    ...
    start_time = ts.utc(2023, 1, 1, 2, 00, 0)

    calculate_genetics_loads(satellite_load_data, start_time.utc_datetime())
    dynamic_groups(satellite_load_data, start_time.utc_datetime(), num_groups)
